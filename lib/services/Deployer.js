  ...
  deployResource(path, options) {
    ...
    return requirements.then(() => {
      if (options.rollingUpdate && resource.type === 'replicationcontrollers') {
        return this.kubectl.rollingUpdate(this.namespace.name, options.rollingUpdate, resource.value);
      }
      ...
      if (options.restart && resource.type === 'replicationcontrollers') {
        return this.kubectl.deleteByLabels(this.namespace.name, 'pod', resource.value.spec.selector);
      }
    });
  }
